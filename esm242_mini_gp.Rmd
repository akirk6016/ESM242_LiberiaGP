...
harvest_dwf <- function(choice, int_state, alpha, p, c_d, q, r, K, discount, period, proportion) {
...
constraint_dwf <- function(choice, int_state, alpha, p, c_d, q, r, K, discount, period, proportion) {
  ...
  for(i in 2:period) {
    ...
    state[i]=state[i-1]+(state[i-1]*r*(1-(state[i-1]/K)))-choice[i-1]
    const[i]=choice[i]-(state[i]*proportion)
  }
...
harvest_ssf <- function(choice, int_state, p, c_s, q, r, K, discount, period, proportion) {
...
constraint_ssf <- function(choice, int_state, p, c_s, q, r, K, discount, period, proportion) {
  ...
  for(i in 2:period){
    ...
    state[i]=state[i-1]+(state[i-1]*r*(1-(state[i-1]/K)))-choice[i-1]
    const[i]=choice[i]-(state[i]*(1-proportion))
  }
  return(const)
}

benefits_fct <- function(choice, Hd_fct, constraintd_fct, Hs_fct, constraints_fct, discount, period, beta, Pi_s){
  ...
  for(i in 1:period) {
    benefits[i]=((1-beta)*choice*(nloptr(x0 = rep(500000, period),eval_f = Hd_fct,lb = rep(0, period),opts = options,eval_g_ineq = constraintd_fct,int_state = 60000000,alpha = choice,p = 1.3,c_d = 0.63,q = 0.01,r = 0.45,K = 100000000,discount = 0.05,period = 50,proportion=0.6)$objective)) + (beta*(nloptr(x0=rep(8000000, period),eval_f=Hs_fct,lb=rep(0,period),opts=options,eval_g_ineq=constraints_fct,int_state=60000000,p=1.3,c_s=0.91,q=0.01,r=0.45,K=100000000, discount=0.05,period=50,proportion=0.6)$objective))
  }
