...
\Pi_{D}=\sum_{t=0}^{T} \rho^{t} (p (1-\alpha) Y_{t,D} - c_{D} (\frac{Y_{t,D}}{q_DX_{t,D}})) \\
...
\Pi_{S}=\sum_{t=0}^{T} \rho^{t} (p (1-\alpha) Y_{t,S} - c_{S} (\frac{Y_{t,S}}{q_SX_{t,S}})) \\
...
benefits_fct <- function(choice, Hd_fct, constraintd_fct, Hs_fct, constraints_fct, discount, period, beta){
  ...
  rho=1/(1+discount)
    ...
    benefits=((1-beta)*choice*-(nloptr(x0 = rep(500000, period),eval_f = Hd_fct,lb = rep(0, period),opts = options,eval_g_ineq = constraintd_fct,int_state = 60000000,alpha = choice,p = 1.3,c_d = 0.63,q = 0.01,r = 0.45, Kd=100000000*0.5,discount = 0.05,period = 50,proportion=0.5)$objective)) + (beta*-(nloptr(x0=rep(500000, period),eval_f=Hs_fct,lb=rep(0,period),opts=options,eval_g_ineq=constraints_fct,int_state=60000000,p=1.3,c_s=0.91,q=0.01,r=0.45,Ks=100000000*0.5, discount=0.05,period=50,proportion=0.5)$objective))
...
yield_plot=ggplot(total_table) +
  geom_point(aes(x=Time, y=Yield, color = Fleet), alpha=0.5) +
  geom_line(aes(x=Time, y=Biomass)) +
  labs(title="SSF Yield and DWF Yield over Time") + 
  theme_bw()
yield_plot
...
benefits_plot=ggplot(total_table) +
  geom_point(aes(x=Time, y=PV, color = Fleet), alpha=0.5) +
  labs(title="SSF Benefits and DWF Benefits over Time") + 
  theme_bw()
benefits_plot
```
